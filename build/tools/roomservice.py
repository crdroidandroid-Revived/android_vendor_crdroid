...
if not depsonly:
    print("Device %s not found. Attempting to retrieve device repository from crDroid Android Github (http://github.com/crdroidandroid)." % device)
...
if not depsonly:
    githubreq = urllib.request.Request("https://api.github.com/search/repositories?q=%s+user:crdroidandroid+in:name+fork:true" % device)
    add_auth(githubreq)
    try:
...
def add_to_manifest(repositories, fallback_branch = None):
    ...
    for repository in repositories:
        ...
        if repo_name.startswith('android_device_'):
            repo_name = 'crdroidandroid/' + repository['repository']
        ...
        project = ElementTree.Element("project", attrib = { "path": repo_target,
            "remote": "crdroid", "name": "%s" % repo_name })
